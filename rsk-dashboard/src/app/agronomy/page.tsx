    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-grow flex flex-col items-center justify-start">
        ...
        {isEditing ? (
          ...
          <div className="w-full max-w-xl bg-white border border-slate-200 p-6 sm:p-8 rounded-3xl shadow-sm space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="mt-1.5 w-full bg-white border border-slate-300 focus:border-slate-500 rounded-xl px-4 py-2.5 text-sm text-slate-800 focus:outline-none transition duration-150"
                    required
                  />
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-2">
                    <div>
                      ...
                      <input type="number" step="0.0001" value={soilLat} onChange={(e) => setSoilLat(e.target.value)}
                        className="w-full bg-white border border-slate-300 focus:border-slate-550 rounded-xl px-3 py-2 text-xs text-slate-800 focus:outline-none transition" />
                    </div>
                    <div>
          ...
          <div className="w-full max-w-2xl space-y-6 animate-fade-in">
            ...
            <div className="bg-white border border-slate-200 p-4 rounded-2xl flex items-center justify-between gap-3 shadow-sm">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-2 bg-white text-slate-700 border border-slate-200 hover:border-slate-350 rounded-xl text-xs font-bold transition cursor-pointer shadow-sm"
              >
                ...
                {recommendationResult.recommendations.map((rec: any, index: number) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`p-4 rounded-2xl space-y-2 relative overflow-hidden cursor-pointer transition-all duration-200 border ${
                        isSelected 
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          {isSelected && <span className="bg-emerald-500 text-white p-0.5 rounded-full"><Check className="h-3 w-3" /></span>}
                        </div>
                      ...
                      <div className="space-y-1.5 pt-1">
                        {rec.suitability_reasons.slice(0, 2).map((reason: string, rIdx: number) => (
                          <div key={rIdx} className="flex gap-1.5 text-[11px] text-slate-650 leading-normal">
            ...
            {matchedMarketData ? (
              <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm space-y-4 animate-fade-in">
                <h4 className="text-xs font-extrabold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                  <TrendingUp className="h-4 w-4 text-slate-700" /> Mandi Pricing Intelligence ({matchedMarketData.crop_name})
                </h4>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                    <span className="text-[10px] text-slate-400 block font-bold">Mandi Price (Quintal)</span>
                    <span className="text-xl font-black text-slate-900">₹{matchedMarketData.market_prices.average_price_per_quintal.toLocaleString()}</span>
                  ...
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                    ...
                    <span className="text-xs font-bold text-slate-800 block truncate">{matchedMarketData.logistics.nearest_mandi_name}</span>
                    <span className="text-[10px] text-slate-400 block pt-1 border-t border-slate-100 mt-1">
                ...
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 text-xs">
                  <span className="font-bold text-slate-500 block mb-0.5">Price Trend: <strong className="text-emerald-700 font-semibold">{matchedMarketData.market_analytics.trend}</strong></span>
                  <p className="text-slate-650 italic">"{matchedMarketData.market_analytics.optimal_selling_window_forecast}"</p>
                </div>
              </div>
            ) : (
              <div className="bg-white border border-slate-200 p-8 rounded-2xl text-center text-xs text-slate-400 shadow-sm">
            ...
            {matchedSchemesData && matchedSchemesData.schemes && matchedSchemesData.schemes.length > 0 && (
              <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm space-y-3 animate-fade-in">
                <h4 className="text-xs font-extrabold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                  <FileText className="h-4 w-4 text-slate-700" /> Matched Government Schemes
                </h4>
                <div className="space-y-2">
                  {matchedSchemesData.schemes.map((sch: any, sIdx: number) => (
                    <div key={sIdx} className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-1">
                      <span className="text-xs font-bold text-emerald-750">{sch.scheme_name}</span>
                      <p className="text-[11px] text-slate-600 leading-normal">{sch.description}</p>
                      <div className="text-[10px] text-slate-400 pt-1 border-t border-slate-100 flex justify-between items-center mt-1">
                        <span>Benefit: <strong className="text-slate-700">{sch.benefit}</strong></span>
                      </div>
                    </div>
                  ))}
                </div>
